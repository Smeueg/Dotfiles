...
# @description Updates Termux's color
# @noargs
update_termux_color() {
    if [ "${TERMUX_VERSION}" ]; then
        file="${HOME}/.termux/colors.properties"
        printf "[\033[1;32mINFO\033[m] Overwriting colors.properties\n"
        printf '# Generated by `update_termux_color()`\n' > "${file}"
        while IFS=" =" read -r key value; do
            [ "${key%%#*}" ] || continue
            printf "%s=%s\n" "${key}" "${value}" >> "${file}"
        done < "${HOME}/.config/theme.ini "
    else
        printf "[\033[1;31mERR\033[m] Not in termux\n"
    fi
}
